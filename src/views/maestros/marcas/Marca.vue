...
const BreadcrumbList = ref([
  ...
  { name: "Marcas", icon: "fas fa-business-time" },
]);
...
const _regMap=ref<Map<number,FormMarca>>()

const Form = ref<FormMarca>({
  slug: "",
  nombre: "",
  deleted:false,
  estado:true
...
async function  enviarRegistros(){
  ...
  Loading.value=true
  try {
    const result=await MarcaService.fnPostRegistrar(form)
    console.log(result)
...
async function _fnConfirmacion(
  ...
  if (result.isConfirmed) {
    ...
    MarcaService.fnPostRegistrar(data)
      .then((res) => {
    ...
    <q-card-section>
      <div class="card-title flex align-items-center">
        <h6 class="flex-grow-1">
          <span class="fas fa-business-time"></span> Marcas
        </h6>
        <button
          type="button"
  ...
  <ModalComponent :active="ActiveModal" :show-footer="false">
    <template #header>
      <div class="modal-header py-3">
        <h6 class="modal-title">{{Form.id?'EDITAR MARCA':'NUEVA MARCA'}}</h6>
      </div>
    </template>
